  ...
  plotAdjacent <- function(simulations, title = NULL, xlab = NULL, ylab = NULL, legend = NULL,
    ...
    if(length(title)!=2 & is.null(title)==F) {
      warning("Title should be provided for both graphs as vector")
    }

    #Check if labels provided for both graphs
    if(length(xlab)!=2 & is.null(xlab)==F) {
      warning("X labels should be provided for both graphs as vector")
    }

    if(length(ylab)!=2 & is.null(ylab)==F) {
      warning("Y labels should be provided for both graphs as vector")
    }

    #Check if legends provided for both graphs
    if(length(legend)!=2 & is.null(legend)==F) {
      warning("Legend should be provided for both graphs as vector")
    }
    ...
    # Create data frame of mean
    mean.df<- as.data.frame(simulations@Mean, xy = T)

    # Create data frame of standard deviation
    std.df<- as.data.frame(simulations@Standard.Deviation, xy = T)

    # Set colour palette for mean and standard deviation
    if (is.null(mean_col)){
    ...
    # Default legend labels for continuous data
    if (is.null(legend)){
    ...
    if (hasValues(simulations@Most.Likely.Class)) {
      ...
      # Create data frame of most likely class
      mlc.df<- as.data.frame(simulations@Most.Likely.Class, xy = T)

      # Create data frame of the probabilities
      ...
      number <- maxValue(simulations@Most.Likely.Class)
      if (is.null(mean_col)){
        mean_col <-colorRampPalette(c("black", "blue", "red"))(number)
      }
